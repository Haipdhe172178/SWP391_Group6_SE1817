...
public class FixNewsControllers extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("edit".equals(action)) {
            ...
            String imgFilename = (imgPart != null && imgPart.getSize() > 0) ? validateAndUploadFile(imgPart, request) : request.getParameter("currentImg");

            if (imgPart != null && imgPart.getSize() > 0 && imgFilename == null) {
                News news = newsDao.getNewsById(id);
                List<Topic> topics = newsDao.getTopic();
                request.setAttribute("news", news);
                request.setAttribute("topics", topics);
