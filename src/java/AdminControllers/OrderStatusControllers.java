...
public class OrderStatusControllers extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (!customerOrders.isEmpty()) {
        statusName = customerOrders.get(0).getStatus().getStatusName();
    } else if (!guestOrders.isEmpty()) {
        statusName = guestOrders.get(0).getStatus().getStatusName();
    }

    request.setAttribute("customerOrders", customerOrders);
    request.setAttribute("guestOrders", guestOrders);
