...
public class ManageStaffControllers extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        int count = accountDAO.getTotalAccountWithRole();
        int endPage;
        account = accountDAO.pagingStaff(index);
        String searchKeyword = request.getParameter("s");
        if (searchKeyword != null &&!searchKeyword.isEmpty()) {
            account = accountDAO.searchAccountRole(index, searchKeyword);
            count = accountDAO.getTotalAccountsByKeywordWithRole(searchKeyword);
        }
        endPage = count/5;
        if (count % 5 != 0) {
            endPage++;
